...
type AdminTab = "questions" | "requests" | "members"
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-6 bg-[#0d0d1a] p-1.5 rounded-2xl border border-purple-500/20">
          {([
            { key: "questions" as AdminTab, label: "Вопросы", icon: "MessageSquare" },
            { key: "requests" as AdminTab, label: "Заявки", icon: "ClipboardList" },
            { key: "members" as AdminTab, label: "Участники & Роли", icon: "Users" },
          ]).map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 ${tab === t.key ? "carnival-gradient-bg text-white" : "text-gray-400 hover:text-white"}`}>
